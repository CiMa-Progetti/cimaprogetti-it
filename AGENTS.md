...
This project uses specialized subagents (defined in `.Codex/agents/` and `frontend/.Codex/agents/`). Each has persistent memory and a focused role. The user expects you to delegate to these agents rather than doing everything inline.
...
5. **commands-master** — Handles package management: installing, removing, updating dependencies, resolving conflicts, auditing vulnerabilities. Lives in `frontend/.Codex/agents/`. Runs on Haiku. Does not write application code.
